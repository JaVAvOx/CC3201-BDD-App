...
def InsertarInsumo(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            elemento = True
            data = Carro(id_insumo=id1,insumo1, etapa= etapa1, estado= estado1)
            data.save()
            return render(request, 'prueba/insertarInsumo.html', {'ingresarInsumo_form': form,'tabla':elemento,'id': id1,'insumoAgregado':insumo1,'etapa':etapa1,'estado':estado1})
    # si es GET (o algo diferente) crearemos un formulario en blanco
    else:
        form = insertarInsumo()
...
def generar_tablaCarro():
    with connection.cursor() as cursor:
        cursor.execute("SELECT * FROM public.carro")
        results = namedtuplefetchall(cursor)
    return results

def generar_tablaInsumo():
    with connection.cursor() as cursor:
        cursor.execute("SELECT * FROM public.insumos")
        results = namedtuplefetchall(cursor)
    return results
...
def generar_tablaProceso():
    with connection.cursor() as cursor:
        cursor.execute("SELECT * FROM public.proceso")
        results = namedtuplefetchall(cursor)
    return results

def generar_tablaProveedor():
    with connection.cursor() as cursor:
        cursor.execute("SELECT * FROM public.proveedor")
        results = namedtuplefetchall(cursor)
    return results
